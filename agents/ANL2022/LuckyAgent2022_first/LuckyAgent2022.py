...
class LuckyAgent2022(DefaultParty):
    ...
    def set_parameters(self, opp):
        if not os.path.exists(f"{self.storage_dir}/m_data"):
            self.min = 0.6
            self.e = 0.05
        else:
            rand_num = random.random()
            saved_data = self.return_saved_data('m_data')
            condition_data = self.return_saved_data('c_data')
    ...
    def save_data(self):
        ...
        c_data = {}
        if os.path.isfile(f"{self.storage_dir}/c_data"):
            # OLD
            # dbfile_c = open(f"{self.storage_dir}/c_data", 'rb')
            # c_data = pickle.load(dbfile_c)
            # dbfile_c.close()
            # NEW
            with open(f"{self.storage_dir}/c_data", 'rb') as dbfile_c:
                c_data = pickle.load(dbfile_c)

        if os.path.exists(f"{self.storage_dir}/c_data"):
            os.remove(f"{self.storage_dir}/c_data")
        ...
        # dbfile_c.close()
        # NEW
        with open(f"{self.storage_dir}/c_data", 'ab') as dbfile_c:
            pickle.dump(c_data, dbfile_c)

        m_data = {}
        if os.path.isfile(f"{self.storage_dir}/m_data"):
            # OLD
            # dbfile = open(f"{self.storage_dir}/m_data", 'rb')
            # m_data = pickle.load(dbfile)
            # dbfile.close()
            # NEW
            with open(f"{self.storage_dir}/m_data", 'rb') as dbfile:
                m_data = pickle.load(dbfile)


        if os.path.exists(f"{self.storage_dir}/m_data"):
            os.remove(f"{self.storage_dir}/m_data")
        ...
        # dbfile.close()
        # NEW
        with open(f"{self.storage_dir}/m_data", 'ab') as dbfile:
            pickle.dump(m_data, dbfile)
